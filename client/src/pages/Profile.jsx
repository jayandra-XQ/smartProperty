    ...
    <div className='min-h-screen bg-slate-50'>
        ...
        <div className='max-w-5xl mx-auto px-8 py-14 relative z-10'>
          ...
          {(fileUploadError || (filePerc > 0 && filePerc <= 100)) && (
            <div className={`mt-6 px-5 py-3 rounded-lg text-sm font-medium flex items-center gap-3 max-w-md
              ...
              <span>{fileUploadError ? 'Upload failed — image must be under 2MB' : filePerc < 100 ? `Uploading photo... ${filePerc}%` : 'Photo updated successfully!'}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className='w-full bg-white border-b border-slate-100 shadow-sm'>
        <div className='max-w-5xl mx-auto px-8'>
          <div className='flex'>
            <div className='text-xs font-bold tracking-widest uppercase text-amber-600 border-b-2 border-amber-500 py-4 px-1 mr-8'>Profile</div>
          </div>
        </div>
      </div>
        ...
        <div className='flex flex-col gap-5'>
          ...
          {isAdmin && (
            <div className='bg-[#0d1b2a] rounded-2xl overflow-hidden relative'>
              ...
              <div className='p-7 relative z-10'>
                ...
                <p className='text-slate-400 text-xs leading-relaxed mb-6'>Reach thousands of buyers and renters. Create your listing in minutes.</p>
          ...
          <div className='bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden'>
            ...
            <div className='p-5'>
              ...
              {showListingsError && <p className='text-red-500 text-xs mt-3'>Could not load listings. Please try again.</p>}
            </div>
          </div>
              ...
              {userListings.map((listing, index) => (
                <div key={listing._id} className='flex items-center gap-5 px-8 py-5 hover:bg-slate-50/80 transition-colors duration-150 group'>
                  ...
                  <Link to={`/listing/${listing._id}`} className='flex-1 min-w-0'>
                    <p className='text-slate-800 font-semibold text-sm truncate group-hover:text-amber-600 transition-colors duration-200 mb-0.5'>{listing.name}</p>
